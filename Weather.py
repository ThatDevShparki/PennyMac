...
#weather is a dictionary that will hold all the temperature data in order to help compute.
weather = {}
count = 0
#Process the file and parse the data within to get data we need to calculate minimum temperature spread.
for line in infile:
    list = line.split()
    #read lines that are not empty and contain essential data I need to compute.
    if len(list) >= 5:
...
for Day, MaxMin in weather.items():
    if(Day > 0 ):
    #check to see if I have the most minimum data stored before storing it.
